  ...
  return (
    <div className='Shop'>
      ...
      <div id='product-section' className=' bg-gradient-to-b from-white via-white to-purple-900 h-full'>
        <h1 className='text-3xl md:text-4xl font-semibold text-center mb-4 pb-4'>Products</h1>
        <div className=' grid grid-cols-2 gap-4 w-full md:w-full lg:w-1/2 m-auto h-auto px-4 md:px-12 justify-between items-center'>
          {Products.map(({id, productName, productPrice, productImage }) => {
            ...
            <div key={ id } className=' justify-center items-center mx-auto shadow-lg shadow-purple-600 mb-8 bg-white rounded-lg'>
                ...
                <div className=' flex justify-center pb-2'>
                  <button className=' bg-orange-400 rounded-lg w-36 md:w-40 h-10 ' onClick={() => addToCart(id)}>Add to Cart {cartItemsAmount > 0 && <>({cartItemsAmount})</>} </button>
                </div>
            </div>
        )})}
        </div>
